...
static sf::Text gHud, gSeg; // gSeg used for MSSV characters on snake segments
...
const char* Gfx_SkinName() {
    static const char* N[] = { "Classic","Neon","Rainbow","Stripes","Magma","Gold" };
...
bool Gfx_Init(sf::RenderWindow& window) {
    ...
    gFoodShape = sf::CircleShape((float)TILE * 0.35f);
    gFoodShape.setFillColor(sf::Color(220, 160, 40));

    gObsRect.setSize({ (float)TILE - 2.f,(float)TILE - 2.f });
    ...
    // GameOver background (now use GameOver.png)
    if (gGameOverBgTexture.loadFromFile("assets/UI/GameOver.png")) {
        gGameOverBgSprite.setTexture(gGameOverBgTexture);
        gGameOverBgLoaded = true;
    }
    else {
        gGameOverBgLoaded = false;
    }

    // Load backgrounds for levels
    const char* bgPaths[3] = {
        "assets/Biomes/bg1.png",
        "assets/Biomes/bg2.png",
        "assets/Biomes/bg3.png"
    };
    for (int i = 0; i < 3; ++i) {
        if (gBgTextures[i].loadFromFile(bgPaths[i])) {
            gBgSprites[i].setTexture(gBgTextures[i]);
            gBgLoaded[i] = true;
        }
    }
...
void Gfx_DrawFrame(sf::RenderWindow& window) {
    ...
    // Filled border areas (walls) — makes the wall area clearly visible
    {
        // color of the wall/border; tweak to taste
        sf::Color wallColor(40, 40, 45); // dark gray
        // top
        sf::RectangleShape topRect({ (float)WIN_W, (float)BORDER * TILE });
        topRect.setPosition(0.f, 0.f);
        topRect.setFillColor(wallColor);
        window.draw(topRect);
        // bottom
        sf::RectangleShape bottomRect({ (float)WIN_W, (float)BORDER * TILE });
        bottomRect.setPosition(0.f, (float)(BORDER + BOARD_H) * TILE);
        bottomRect.setFillColor(wallColor);
        window.draw(bottomRect);
        // left
        sf::RectangleShape leftRect({ (float)BORDER * TILE, (float)BOARD_H * TILE });
        leftRect.setPosition(0.f, (float)BORDER * TILE);
        leftRect.setFillColor(wallColor);
        window.draw(leftRect);
        // right
        sf::RectangleShape rightRect({ (float)BORDER * TILE, (float)BOARD_H * TILE });
        rightRect.setPosition((float)(BORDER + BOARD_W) * TILE, (float)BORDER * TILE);
        rightRect.setFillColor(wallColor);
    ...
    for (std::size_t i = 0; i < Game_ObstacleCount(); ++i) {
        ...
        if (gRockLoaded) {
            ...
            gRockSprite.setPosition(p.x + TILE * 0.5f, p.y + TILE * 0.5f);
            window.draw(gRockSprite);
        }
        else {
            gObsRect.setPosition(p.x + 1.f, p.y + 1.f);
    ...
    // Auto-pilot gate indicator (Torii)
    if (Game_IsAutoPilot()) {
        Cell gate = Game_PortalPos();
        sf::Vector2f p = CellToPx(gate.x, gate.y);

        if (toriiLoaded) {
            // Use the cell center as base
            sf::Vector2f center = p;
            center.x += TILE * 0.5f;
            center.y += TILE * 0.5f;

            // fixed small scale so Torii image appears as in your reference
            sprTorii.setScale(0.20f, 0.20f);

            // origin: center horizontally, bottom vertically (so it sits on the ground)
            if (sprTorii.getTexture()) {
                sprTorii.setOrigin(
                    static_cast<float>(sprTorii.getTexture()->getSize().x) / 2.0f,
                    static_cast<float>(sprTorii.getTexture()->getSize().y)
                );
            }

            // position Torii so the portal appears centered under the crossbar
            sprTorii.setPosition(center.x, center.y + TILE * 0.70f);

            window.draw(sprTorii);
        }
        else {
            // fallback: blinking circle
            static sf::CircleShape gateCircle;
            gateCircle.setRadius(TILE * 0.5f);
            gateCircle.setOrigin(TILE * 0.5f, TILE * 0.5f);
    ...
    // Snake + MSSV characters
    bool first = true;
    std::size_t len = Game_SnakeLen();
    std::string mssv = MSSV_STRING;

    // determine if snake currently in ghost/whirlwind state
    ...
    for (std::size_t i = 0; i < len; ++i) {
        ...
        if (isGhost) {
            ...
            if (!mssv.empty()) {
                ...
                // dim text by reducing alpha
                sf::Color txtCol = gSeg.getFillColor();
                gSeg.setFillColor(sf::Color(txtCol.r, txtCol.g, txtCol.b, 160));
                gSeg.setPosition(p.x + (float)TILE / 2.f, p.y + (float)TILE / 2.f);
                window.draw(gSeg);
                // restore to white for next segments (global state)
                gSeg.setFillColor(sf::Color::White);
            }
        }
        else {
    ...
    if (Game_Over()) {
        ...
        // Draw GameOver UI (title image + buttons)
        Gfx_DrawGameOverOverlay(window);
    }

    // If game is paused, draw the pause menu overlay from UI module
    if (Game_Paused()) {
